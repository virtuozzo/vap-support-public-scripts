...
done < <(vzlist -Ho hostname,ctid | awk '($1 ~ /^(jelcore|jbilling|jstatistic|hcore|webgate|gate)/)')
...
for instance in "${!instance_ctid[@]}"; do
  if [[ ${instance_count[$instance]} -gt 1 ]]; then
    ...
    warnmsg "Mentioned containers are currently skipped"
  else
    echo -e "\n################# ${instance_ctid[$instance]} #################"
    vzctl exec2 ${instance_ctid[$instance]} "bash +x /root/scripts/disable_me.sh"
    echo;
  fi
done
